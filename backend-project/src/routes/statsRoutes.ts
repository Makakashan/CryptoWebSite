        ...
        ORDER BY holders DESC`,
			...
			res.json({
				...
				orders: {
					...
					totalVolume:
						Math.round((ordersStats.totalVolume || 0) * 100) / 100,
				},
				mostTradedAssets,
				resentActivity,
				topTraders,
        ...
        GROUP BY asset_symbol`,
			...
			res.json({
				orders: {
					...
					sellOrders: ordersStats.sellOrders || 0,
					totalSpent:
						Math.round((ordersStats.totalSpent || 0) * 100) / 100,
					totalEarned:
						Math.round((ordersStats.totalEarned || 0) * 100) / 100,
				},
				portfolio: {
				...
				performance: {
					...
					profitLossPercent:
						Math.round((profitLoss / 10000) * 10000) / 100,
				},
				ordersByAsset,
			});
		} catch (error) {
