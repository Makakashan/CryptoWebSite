...
import { parsePagination, validateSortField, validateSortOrder, parseNumericFilter, buildPaginationResponse, assetToAssetWithPrice, } from "../utils/helpers.js";
import { ALLOWED_ASSET_CATEGORIES, ALLOWED_ASSET_SORT_FIELDS, mapCoinGeckoCategory, } from "../utils/constants.js";
...
router.get("/", async (req, res) => {
    ...
    const sortBy = validateSortField(req.query.sortBy, ALLOWED_ASSET_SORT_FIELDS, "symbol");
    ...
    const minPrice = parseNumericFilter(req.query.minPrice);
    const maxPrice = parseNumericFilter(req.query.maxPrice);
    try {
        // Build WHERE clause
        const conditions = [];
        const params = [];
        ...
        const countResult = await db.get(`SELECT COUNT(*) as total FROM assets ${whereClause}`, params);
        const total = countResult?.total || 0;
        // Fetch assets
        const isSortingByPrice = sortBy === "price";
        const assets = isSortingByPrice
            ? await db.all(`SELECT * FROM assets ${whereClause}`, params)
            : await db.all(`SELECT * FROM assets ${whereClause} ORDER BY ${sortBy} ${sortOrderSQL} LIMIT ? OFFSET ?`, [...params, limit, offset]);
        ...
        if (isSortingByPrice) {
            assetsWithPrice.sort((a, b) => sortOrder === "asc" ? a.price - b.price : b.price - a.price);
        ...
        if (minPrice !== null || maxPrice !== null) {
           ...
           VALUES (?, ?, ?, ?, ?, ?)`, [symbol, symbol, image_url, category, description, 1]);
            ...
            router.get("/search/:symbol", authenticateToken, async (req, res) => {
                const db = getDB();
                ...
                try {
                    ...
                    await db.run(`INSERT INTO assets (symbol, name, category, is_active)
         VALUES (?, ?, ?, ?)`, [symbol, symbol, "crypto", 1]);
            ...
            router.get("/:symbol/orders", authenticateToken, async (req, res) => {
                const db = getDB();
                let symbol = req.params.symbol.toUpperCase();
                if (!symbol.endsWith("USDT")) {
                    symbol = `${symbol}USDT`;
                ...
                try {
                    ...
                    // Get total count
                    const countResult = await db.get(`SELECT COUNT(*) as total FROM orders WHERE asset_symbol = ?`, [symbol]);
                    const total = countResult?.total || 0;
                    // Get orders with user information
                    const orders = await db.all(`SELECT
          o.*,
          u.username
        FROM orders o
        ...
        LIMIT ? OFFSET ?`, [symbol, limit, offset]);
                    const assetWithPrice = assetToAssetWithPrice(asset);
            ...
            router.get("/:symbol/holders", authenticateToken, async (req, res) => {
                const db = getDB();
                let symbol = req.params.symbol.toUpperCase();
                if (!symbol.endsWith("USDT")) {
                    symbol = `${symbol}USDT`;
                ...
                try {
                    ...
                    const countResult = await db.get(`SELECT COUNT(*) as total FROM portfolio WHERE asset_symbol = ?`, [symbol]);
                    const total = countResult?.total || 0;
                    // Get holders with user information
                    const holders = await db.all(`SELECT
          u.id,
          u.username,
          u.balance,
        ...
        LIMIT ? OFFSET ?`, [symbol, limit, offset]);
                    ...
                    const response = buildPaginationResponse(holdersWithValue, page, limit, total, "amount", "desc");
                    res.json({
                        asset: assetWithPrice,
                        holders: response,
                    });
            ...
            router.get("/:symbol", async (req, res) => {
                ...
                try {
                    ...
                    const orderStats = await db.get(`SELECT
        ...
        WHERE asset_symbol = ?`, [symbol]);
                    const holdersCount = await db.get(`SELECT COUNT(DISTINCT user_id) as holders
        FROM portfolio
        WHERE asset_symbol = ?`, [symbol]);
                    res.json({
                        asset: assetWithPrice,
            ...
            router.post("/", authenticateToken, async (req, res) => {
                ...
                try {
                    const existingAsset = await db.get("SELECT * FROM assets WHERE symbol = ?", [normalizedSymbol]);
                    if (existingAsset) {
                        res
                            .status(400)
       ...
       VALUES (?, ?, ?, ?, ?, ?)`, [
            ...
            router.put("/:symbol", authenticateToken, async (req, res) => {
                const db = getDB();
...
//# sourceMappingURL=assetsRoutes.js.map