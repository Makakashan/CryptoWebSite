...
router.get("/", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const assets = await db.all("SELECT asset_symbol, amount FROM portfolio WHERE user_id = ?", [userId]);
        res.json({
            id: userId,
...
router.put("/:asset_symbol", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await db.run("UPDATE portfolio SET amount = ? WHERE user_id = ? AND asset_symbol = ?", [amount, userId, asset_symbol]);
...
router.delete("/:asset_symbol", authenticateToken, async (req, res) => {
    ...
    const { asset_symbol } = req.params;
    try {
        const existingAsset = await db.run("DELETE FROM portfolio WHERE user_id = ? AND asset_symbol = ?", [userId, asset_symbol]);
