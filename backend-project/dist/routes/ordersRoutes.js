...
import { parsePagination, validateSortField, validateSortOrder, parseNumericFilter, parseDateFilter, buildPaginationResponse, } from "../utils/helpers.js";
import { ALLOWED_ORDER_TYPES, ALLOWED_ORDER_SORT_FIELDS, mapCoinGeckoCategory, } from "../utils/constants.js";
const router = express.Router();
async function ensureAssetExists(asset_symbol) {
    const db = getDB();
    ...
    try {
        const binanceResponse = await fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${asset_symbol}`);
        ...
        try {
            ...
            if (coinsListResponse.ok) {
                ...
                if (coin) {
                    const coinDetailResponse = await fetch(`https://api.coingecko.com/api/v3/coins/${coin.id}`);
        ...
        await db.run(`INSERT INTO assets (symbol, name, image_url, category, description, is_active) VALUES (?, ?, ?, ?, ?, ?)`, [asset_symbol, asset_symbol, image_url, category, description, 1]);
...
router.post("/place", authenticateToken, async (req, res) => {
    ...
    try {
        if (order_type === "BUY") {
            ...
            const existingAsset = await db.get("SELECT * FROM portfolio WHERE user_id = ? AND asset_symbol = ?", [userId, asset_symbol]);
            if (existingAsset) {
                await db.run("UPDATE portfolio SET amount = amount + ? WHERE user_id = ? AND asset_symbol = ?", [amount, userId, asset_symbol]);
            }
            else {
                await db.run("INSERT INTO portfolio (user_id, asset_symbol, amount) VALUES (?, ?, ?)", [userId, asset_symbol, amount]);
            }
        }
            ...
            if (existingAsset.amount - amount <= 0) {
                await db.run("DELETE FROM portfolio WHERE user_id = ? AND asset_symbol = ?", [userId, asset_symbol]);
            }
        }
        else {
            res
        ...
        await db.run("INSERT INTO orders (user_id, asset_symbol, order_type, amount, price_at_transaction) VALUES (?, ?, ?, ?, ?)", [userId, asset_symbol, order_type, amount, price]);
        res.json({
...
router.get("/history", authenticateToken, async (req, res) => {
    ...
    const sortBy = validateSortField(req.query.sortBy, ALLOWED_ORDER_SORT_FIELDS, "timestamp");
    const sortOrder = validateSortOrder(req.query.sortOrder);
    const sortOrderSQL = sortOrder.toUpperCase();
    const assetSymbol = req.query.asset_symbol;
    ...
    const minAmount = parseNumericFilter(req.query.minAmount);
    const maxAmount = parseNumericFilter(req.query.maxAmount);
    try {
        // Build WHERE clause
        ...
        const orders = await db.all(`SELECT * FROM orders ${whereClause} ORDER BY ${sortBy} ${sortOrderSQL} LIMIT ? OFFSET ?`, [...params, limit, offset]);
...
router.delete("/:id", authenticateToken, async (req, res) => {
    ...
    const orderId = req.params.id;
    try {
        const result = await db.run("DELETE FROM orders WHERE id = ? AND user_id = ?", [orderId, req.user.id]);
        if (result.changes === 0) {
            res
                .status(404)
