...
router.get("/", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const userStats = await db.get("SELECT COUNT(*) as totalUsers, SUM(balance) as totalBalance FROM users");
        ...
        FROM orders`);
        const mostTradedAssets = await db.all(`SELECT asset_symbol,
          COUNT(*) as tradeCount,
        ...
        LIMIT 10`);
        const resentActivity = await db.all(`SELECT
        ...
        ORDER BY date ASC`);
        const topTraders = await db.all(`SELECT
          u.id,
          u.username,
        ...
        LIMIT 10`);
        const assetDistribution = await db.all(`SELECT
          asset_symbol,
        ...
        res.json({
            users: {
                ...
                totalBalance: Math.round((userStats.totalBalance || 0) * 100) / 100 || 0,
            },
            orders: {
...
router.get("/user", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const portfolio = await db.all(`SELECT asset_symbol,
          amount
        FROM portfolio
        WHERE user_id = ?`, [userId]);
        ...
        FROM orders
        WHERE user_id = ?
        GROUP BY asset_symbol`, [userId]);
        const netSpent = (ordersStats.totalSpent || 0) - (ordersStats.totalEarned || 0);
