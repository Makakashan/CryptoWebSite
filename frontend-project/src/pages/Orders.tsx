...
const Orders = () => {
	...
	const filteredOrders = (orders || []).filter((order: Order) => {
		const matchesAsset = !assetFilter || order.asset_symbol.toLowerCase().includes(assetFilter.toLowerCase());
						...
						<table className="w-full">
							<thead>
								<tr className="border-b border-white/[0.06]">
									<th className="text-left text-xs font-medium text-white/40 pb-3 pr-4 cursor-pointer hover:text-white/60 transition-colors" onClick={() => handleSort("asset_symbol")}>
								...
								{filteredOrders.map((order: Order, index: number) => (
									...
									>
										<td className="py-3 pr-4">
											<span className="text-sm font-medium text-white">{order.asset_symbol}</span>
										</td>
										<td className="py-3 pr-4">
											<span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-semibold ${
												...
												{order.order_type === "BUY" ? <ShoppingCart className="w-3 h-3" /> : <ShoppingCart className="w-3 h-3" />}
												{order.order_type}
											</span>
										</td>
										<td className="text-right text-sm text-white/60 py-3 pr-4">{order.amount.toLocaleString()}</td>
										<td className="text-right text-sm text-white/60 py-3 pr-4">${order.price_at_transaction.toFixed(2)}</td>
										<td className="text-right text-xs text-white/40 py-3">{formatDate(order.timestamp)}</td>
									</motion.tr>
								))}
								{filteredOrders.length === 0 && (
									<tr>
										<td colSpan={5} className="text-center text-white/40 py-8">No orders found</td>
									</tr>
								)}
							</tbody>
						</table>
